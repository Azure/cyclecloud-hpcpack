...
powershell_script 'install-hpcpack' do
  ...
  if($vaultName -and $vaultCertName) {
    #{bootstrap_dir}\\InstallHPCBrokerNode.ps1 -SetupFilePath "#{install_dir}\\Setup.exe" -ClusterConnectionString #{node['hpcpack']['hn']['hostname']} -VaultName $vaultName -VaultCertName $vaultCertName -ManagedId '#{node[:hpcpack][:identity]}'
  }
  else {
