...
powershell_script 'Install-HpcSingleHeadNode' do
    ...
    if($vaultName -and $vaultCertName) {
      #{bootstrap_dir}\\InstallHPCHeadNode.ps1 -ClusterName $env:ComputerName -VaultName $vaultName -VaultCertName $vaultCertName -SetupCredential $domainCred -ManagedId '#{node[:hpcpack][:identity]}'
    }
    else {
