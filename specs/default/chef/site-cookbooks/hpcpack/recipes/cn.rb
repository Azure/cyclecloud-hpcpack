...
powershell_script 'install-hpcpack' do
  ...
  if($vaultName -and $vaultCertName) {
    #{bootstrap_dir}\\InstallHPCComputeNode.ps1 -SetupFilePath $setupFilePath -ClusterConnectionString #{node['hpcpack']['hn']['hostname']} -VaultName $vaultName -VaultCertName $vaultCertName -ManagedId '#{node[:hpcpack][:managedIdentity]}'
  }
  else {
